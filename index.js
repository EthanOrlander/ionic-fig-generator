...
var attachSubcommand = function (completionSpec, commandStr) {
    ...
    if (!last)
        throw Error('Err');
    var _loop_1 = function (subcommand) {
        if (!last.subcommands)
            last.subcommands = [];
        if (!last.subcommands.find(function (sc) { return sc.name === subcommand; }))
            last.subcommands.push({ name: subcommand });
        last = last.subcommands.find(function (sc) { return sc.name === subcommand; });
    };
    for (var _i = 0, commandPieces_1 = commandPieces; _i < commandPieces_1.length; _i++) {
        var subcommand = commandPieces_1[_i];
        _loop_1(subcommand);
    }
    return last;
};
var generateSpec = function (commands) {
    var completionSpec = {
        name: 'ionic',
